...
//   const moveSpeed = 5; // Speed for translating the canvas
//   const boundaryOffset = 100; // Distance from the canvas edges to trigger movement
...
// Connect to the Socket.IO server
// const socket = io("http://localhost:3000");
const socket = io("https://agar-io-backend.onrender.com/");
...
const Game = () => {
  ...
  // Emit attack event on space key press
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.code === "Space") {
        e.preventDefault(); // Prevent scrolling on space press
        socket.emit(SocketEvents.Attack, { playerId: currentPlayer?.id });
        console.log("attack");
      }
    };
